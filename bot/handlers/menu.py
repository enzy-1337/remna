...
async def cb_service_info(cq: CallbackQuery, db_user: User | None) -> None:
    ...
    cap = join_lines(
        ...
        "",
        plain(
            "Здесь вы можете оформить подписку, продлить её, пополнить баланс, "
            "управлять устройствами и смотреть детализацию списаний. "
            "Сервис работает по модели гибридного биллинга: часть расходов может "
            "покрываться пакетом тарифа, а перерасход списывается отдельно."
        ),
        plain(
            "После активации подписки вы получаете ссылку и QR для быстрого подключения "
            "в приложениях на телефоне и ПК. В профиле доступны история платежей, "
            "реферальная программа и управление автопродлением."
        ),
        plain(
            "Если у вас есть промокод, активируйте его в соответствующем разделе — "
            "скидка или бонус применятся по правилам промокода."
        ),
        plain(
            "В базовую подписку уже входят 2 устройства; при необходимости можно "
            "докупить слоты и пользоваться до 10 устройствами — например, телефон, "
